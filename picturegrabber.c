...
void capture_std(capture_device *pcapt_d)
{
  int err;
...
void capture_stop(capture_device *pcapt_d, int streaming)
{
  ...
  if (pcapt_d->fd_video >= 0)
  {
    if (streaming)
    {
      i = V4L2_BUF_TYPE_SDR_CAPTURE;
      int err = ioctl(pcapt_d->fd_video, VIDIOC_STREAMOFF, &i);
...
int capture_start(capture_device *pcapt_d, int streaming)
{
  ...
  /* Get Video Format */
  pcapt_d->fmt.type = V4L2_BUF_TYPE_SDR_CAPTURE;
  ...
  if (streaming)
  {
    ...
    pcapt_d->req.type = V4L2_BUF_TYPE_SDR_CAPTURE;
    ...
    for (i = 0; i < pcapt_d->req.count; ++i)
    {
      ...
      vidbuf->index = i;
      vidbuf->type = V4L2_BUF_TYPE_SDR_CAPTURE;
      ...
      pcapt_d->vimage[i].data = mmap(0, vidbuf->length,
                                     ...
                                     pcapt_d->fd_video, vidbuf->memory);
...
int capture_init(capture_device *pcapt_d, char *my_device,
...
int capture_dq(capture_device *pcapt_d,
...
void capture_q(capture_device *pcapt_d,
               struct v4l2_buffer *pbuf,
               int streaming)
{
  int err;
...
void grabSetFps(int fd, int fps)
{
  ...
  params.type = V4L2_BUF_TYPE_SDR_CAPTURE;
...
int export_jpeg(capture_device *pcapt_d, int count)
{
  ...
  snprintf(fileout, 50, "%s%s", "output", ".jpg");
